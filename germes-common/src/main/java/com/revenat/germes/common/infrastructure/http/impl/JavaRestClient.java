package com.revenat.germes.common.infrastructure.http.impl;
...
public class JavaRestClient implements RestClient {
    ...
    public JavaRestClient(final int timeout, final JsonTranslator jsonTranslator) {
        Asserts.asserts(timeout > 0, "timeout can not be negative value:%d", timeout);
        Asserts.assertNotNull(jsonTranslator,  "json client can not be null");
    ...
    public <T> RestResponse<T> post(final String url, final Class<T> clz, final Object entity) {
        ...
        final String json = jsonTranslator.toJson(entity);
    ...
    private <T> RestResponse<T> execute(final HttpRequest request, final Class<T> responseBodyType) {
        try {
            ...
            if (isError(response)) {
                throw new HttpRestException(response.body(), response.statusCode());
            }

            final String text = response.body();
            final T body = StringUtils.isEmpty(text) ? null : jsonTranslator.fromJson(text, responseBodyType);
